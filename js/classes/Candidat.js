...
export default class Candidat extends User {
  ...
  static sendQuise = async () => {
    ...
    if (points == 1) {
      ...
      Swal.fire({
        ...
        text:
          "Vous Être répondre  sur  " +
          points * this.allQuises.length +
          "/" +
          this.allQuises.length +
          "  de questions",
        position: "center",
        icon: "success",
      ...
      Swal.fire({
        ...
        text:
          "Vous Être répondre seulement sur  " +
          points * this.allQuises.length +
          "/" +
          this.allQuises.length +
          "  de questions",
  ...
  static sendTestTechnique = async () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        // let email1 = sessionStorage.getItem("username");

        // let uri1 = `http://localhost:5001/users?email= ${email1}`;

        // const res1 =  fetch(uri1);
        // const candidat =  res1.json();

        //  fetch(
        //   "http://localhost:5001/users/" + candidat[0].id,
        //   { state: true },
        //   {
        //     method: "PATCH",
        //     headers: { "Content-Type": "application/json" },
  ...
  static CheckAuthentificationCandidat = async () => {
    ...
    if (
      !candidat.length == 1 ||
      !candidat[0].email === email ||
      candidat[0].state == true
    ) {
      window.location.replace("/PageLogin.html");
    }
  };
